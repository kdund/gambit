...
namespace Gambit
{
    ...
    namespace Scanner
    {
        ...
        namespace Python
        {
            ...
            class printer_wrapper : public printer_interface
            {
            private:
                python_interface *scanner;
                
            public:
                printer_wrapper(python_interface *scanner) : scanner(scanner) {}
                
                void aux_printer(const std::string &key, const double &value);
                
                void main_printer(const std::string &key, const double &value);
                
                void main_printer(std::unordered_map<std::string, double> &key_map);
            };
            
            class python_interface : public scan_interface
            {
            private:
            ...
            public:
                ...
                {
                    return std::shared_ptr<printer_interface>(new printer_wrapper(this), [](printer_interface *in){delete in;});
                }
                ...
                int run_scan_node(YAML::Node *node, const Gambit::Scanner::Factory_Base *factory, Gambit::Priors::BasePrior *user_prior, bool resume)
                {
                ...
                int run_scan_str(std::string *filename, const Gambit::Scanner::Factory_Base *factory, Gambit::Priors::BasePrior *user_prior, bool resume)
                {
                    IniFileParser iniFile;
...
typedef Gambit::Scanner::Python::python_interface pyinterface;
typedef Gambit::Scanner::Python::scan_interface pyinterface_base;

extern "C"
{

    std::shared_ptr<pyinterface_base> get_gambit_interface()
    {
        return std::shared_ptr<pyinterface_base>(new pyinterface(), [](pyinterface_base *in){delete in;});
    }
